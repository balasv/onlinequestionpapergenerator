...
if(isset($_POST['submit']))
{
 ...
 $stmt = $user->runQuery("SELECT custid FROM customer WHERE email=:email LIMIT 1");
 $stmt->execute(array(":email"=>$email));
 $row = $stmt->fetch(PDO::FETCH_ASSOC); 
 if($stmt->rowCount() == 1)
 {
  $id = base64_encode($row['custid']);
  $code = md5(uniqid(rand()));
  
  $stmt = $user->runQuery("UPDATE customer SET tokenCode=:token WHERE email=:email");
